...
async function proxyMiddleware(req: NextRequest) {
    if (req.nextUrl.pathname.startsWith('/__clerk')) {
        ...
        // Forward headers with Clerk-specific headers
        const proxyHeaders = new Headers()
        req.headers.forEach((value, key) => {
            proxyHeaders.set(key, value)
        })

        // These headers are required for Clerk to recognize the proxy
        ...
        console.log(`[Clerk Proxy] Proxying to ${clerkUrl.toString()}`)
        ...
        try {
            ...
            console.error(`[Clerk Proxy] Fetch error:`, error)
